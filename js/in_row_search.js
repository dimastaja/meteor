...
$(function () {
    ...
    function getData()
    {
        var getData;
        $.ajax({
            url: '/datasend/in_row_search.php?ID='+ID+'&num='+num,
            method: 'GET',
            async: false,
            dataType: 'json',
     ...
     $(".builder").click(function (){
        ...
        for(var i=0; i<=3; i++)        
        {
            ...
            MainPlot=$.plot($('.graphWrapper').find('.placeholder1:eq('+plotNum+')'), startData, options);
            //SecondPlot=$.plot($('.graphWrapper').find('.placeholder2:eq('+plotNum+')'), drop, options2);
            plotNum++;
            num++;
        }
        ID++;
    ...
    {
        $.get(
            "/datasend/delete_from_in_row.php?ID="+ID,{},function (data){$('.builder').click()}
        );
