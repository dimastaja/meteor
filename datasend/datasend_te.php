			...
			foreach ($arField as $field)
			{
				...
				if ((log10($row[$field])>=$min)&&(log10($row[$field])<=$max))
				{
				  //printr($sec);
                                    ...
                                    //   printr($dropTime);
                                    $arData[$field][]=array(($sec-$dropTime),round(log10($row[$field]),5));
                                    $arTime[$row["Date"]]=$time++;
                                   ...
                                   // printr($secPr);
                                    ...
                                    elseif(!$firstParab)
                                    {
                                        $firstParab=true;
                                        if ($countParab>0)
                                        $arData['PARAB'][$countParab][]=array('SEC'=>($sec-12000),'LOG'=>round(log10($arRows[$rowCount-2][$field]),5),'D1'=>$arRows[$rowCount-2][$field]);
                                    }
                                    ...
                                    if ($firstParab&&$field=='D1'&&$_GET['showDrop'])
                                    {
                                        ...
                                        $arData['PARAB'][$countParab][]=array('SEC'=>($sec-$dropTime),'LOG'=>round(log10($row[$field]),5),'D1'=>$row[$field]);
                                    }
   ...
   // printr($arData['PARAB']);
    ...
    if ($_GET['showDrop'])
    {
        $query="select Date, Possobility, d1 from errupt_possibility where Date>='$first' and Date<='$last' and Possobility>=0.5 and metka<1 and Possobility<=1  order by date $order  ";
       // printr($query);
       // return;
        $res=$db->query($query);
        $dropInd=0;
        ...
        {
            $sec=(strtotime($row["Date"]))*1000;
            //$arDrops[][]=array($sec, round(log10($row['d1']),5));
            $num=$arTime[$row["Date"]];
           // printr($arData['D1'][$num]);
            
            for($idr=$num-5; $idr<=$num+5;$idr++)
                $arDrops[$dropInd][]=$arData['D1'][$idr];
            if (abs($arDrops[$dropInd][0][1]-$arDrops[$dropInd][6][1])<1)
            {
                unset($arDrops[$dropInd--]);
            }
            else
            {//printr(abs($arDrops[$dropInd][0][1]-$arDrops[$dropInd][6][1]));
               // printr($arDrops[$dropInd]);
                
            }
            
           // printr(array($sec, round(log10($row['d1']),5)));
            $countDrop++;
            $dropInd++;
        }
        $sec=$arDrops[0][0][0];
        ...
        {
            foreach($arDrop as $drop)
            {
                $arDataDrop[]=array($sec, $drop[1]);
                $sec=$sec+12000;
            }
            $arDataDrop[]=array($sec, 1);
            $sec=$sec+12000;
            $arDataDrop[]=array($sec, 1);
            $sec=$sec+12000;
            $arDataDrop[]=array($sec, 1);
 ...
 // printr($arData);
    ...
    //printr($arData['PARAB']);
    $dataSend["PLOT"][]=array('label'=>'drops', 'data'=>$arDataDrop, 'color'=>'#ff0000');
