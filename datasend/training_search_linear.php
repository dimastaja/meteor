    ...
    {
        $query="select * from data_2 where date>'2003-07-24 20:10:19'order by  date asc limit 1";
        $res=$db->query($query);
        $row =$res->fetch_assoc();
        if ($row['Date'])
            $first=$row['Date'];
       ...
       $filterQuery.=" and d1>0 and L<7.1 and L>2.5 ";
    if ($_GET['back'])
               ...
               // if ((($dif<0)&&($arPoint['SEC']<$arMax['SEC']))||(($dif>0)&&($arPoint['SEC']>$arMax['SEC'])))
                {
                    ...
                    while($rowDrop =$resDrop->fetch_assoc())
                    {
                         ...
                         $arDrop[]=array('SEC'=>$sec,'D1'=>$rowDrop['d1'], "LOG"=>round(log10($rowDrop['d1']),15));
                    }
                   // printr($arDrop);
